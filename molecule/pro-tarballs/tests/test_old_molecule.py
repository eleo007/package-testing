...
def test_load_env_vars_define_in_test(host):
    ...
    with host.sudo():
        vars={'BASE_DIR':'/usr/percona-server','PS_VERSION':os.getenv('PS_VERSION'),'PS_REVISION':os.getenv('PS_REVISION'),'PRO':os.getenv('PRO'),'FIPS_SUPPORTED':os.getenv('FIPS_SUPPORTED')}
...
def test_bats(host, test_load_env_vars_define_in_test):
    cmd = f"{vars} /home/vagrant/package-testing/binary-tarball-tests/ps/run.sh"
# with host.sudo():
    result = host.run(cmd)
    print(result.stdout)
    print(result.stderr)
