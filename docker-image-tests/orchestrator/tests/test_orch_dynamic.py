...
def test_source(prepare, value, key1, key2):
...
#    for value in source_state_check:
    if key2:
        if key1 == 'SecondsSinceLastSeen': # Lastseen is int and should be less than 7 sec
            assert value > parced_source_state[key1][key2], value
            ...
            assert value == parced_source_state[key1][0][key2], value
        else: # All other cases.
            assert value == parced_source_state[key1][key2], value
    elif not key2:
        assert value == parced_source_state[key1], value
    else:
